...
import { QUIZ_QUESTIONS } from './appSoftwareData';
...
export default function QuizSummarySection() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      ...
      >
        <h3 style={{ fontSize: '1.4rem', color: 'var(--color-text)', margin: '0 0 6px 0' }}>
          💻 Application Software: Final Summary
        </h3>
        <p style={{ color: 'var(--color-text-muted)', fontSize: '0.95rem', maxWidth: '600px', margin: '0 auto 20px auto' }}>
          Application software helps users perform specific tasks. Remember the two key types:
        </p>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '14px', textAlign: 'left' }}>
          <div style={{ background: 'var(--color-surface-raised)', padding: '16px', borderRadius: '12px', borderLeft: '4px solid #3b82f6' }}>
            ...
            <p style={{ margin: '4px 0 0 0', fontSize: '0.85rem', color: 'var(--color-text-muted)' }}>
              <strong>Ready-made</strong> for many people and general needs (e.g. Word processors, Spreadsheets, Drawing apps, Web browsers).
            </p>
          </div>

          <div style={{ background: 'var(--color-surface-raised)', padding: '16px', borderRadius: '12px', borderLeft: '4px solid #8b5cf6' }}>
            <div style={{ fontSize: '1.8rem' }}>🛠️</div>
            <strong style={{ color: '#8b5cf6', fontSize: '1.05rem' }}>Tailored Software</strong>
            <p style={{ margin: '4px 0 0 0', fontSize: '0.85rem', color: 'var(--color-text-muted)' }}>
              <strong>Custom-made</strong> according to a specific user&apos;s or organization&apos;s requirements (e.g. School, Hospital, Banking systems).
            </p>
          </div>
        </div>
      </div>
      ...
      {!isFinished ? (
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--color-border)', paddingBottom: '10px' }}>
            <h4 style={{ margin: 0, fontSize: '1.2rem', color: 'var(--color-text)' }}>
              🎯 Application Software Quiz (Class 5)
            </h4>
        ...
        >
          ...
          <h3 style={{ fontSize: '1.6rem', color: 'var(--color-text)', margin: '0 0 6px 0' }}>
            {score === QUIZ_QUESTIONS.length ? 'Outstanding! Perfect Score!' : 'Great Effort!'}
          </h3>
